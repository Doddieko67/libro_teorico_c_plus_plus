\chapter{Tu Primer Programa y un Vistazo a la Historia}
\label{chap:hola_mundo}

% Introducción al capítulo, explicando que este es el primer paso práctico para escribir código.

\section{El Ritual de Iniciación: "Hola, Mundo"}
\label{sec:ritual_hola_mundo}
...
\subsection{Nuestro Primer Programa en C++ Moderno}
...
\subsection{Anatomía de Nuestro Programa}
...
\begin{itemize}
    ...
    \item \textbf{\texttt{<<}:} Este es el "operador de inserción". Piensa en él como una flecha que "envía" o "inserta" lo que está a su derecha hacia `std::cout` (la consola).
...
\label{sec:nota_historica}

% 1. Mostrar cómo se hacía en C con printf.
% 2. Explicar brevemente por qué el enfoque de std::cout es superior (principalmente, seguridad de tipos).
